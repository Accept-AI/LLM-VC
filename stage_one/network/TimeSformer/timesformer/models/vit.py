...
class TimeSformerModified(nn.Module):
    def __init__(self, original_model, num_classes_identity, num_classes_action):
        ...
        self.shared_blocks = nn.Sequential(*list(original_model.model.blocks[:2]))  # 前四个Block作为共享特征提取层

        # 分别定义两个分支，从原始模型中提取剩余Block
        self.identity_branch = nn.Sequential(*list(original_model.model.blocks[2:]))  # 用于身份预测的分支
        self.action_branch = nn.Sequential(*list(original_model.model.blocks[2:]))  # 用于动作预测的分支
        ...
        # ori visiontransformer
        # self.model = VisionTransformer(img_size=original_model.model.img_size, num_classes=original_model.model.num_classes, patch_size=original_model.model.patch_size, embed_dim=768,
        #                                depth=12, num_heads=12, mlp_ratio=4, qkv_bias=True,
        #                                norm_layer=partial(nn.LayerNorm, eps=1e-6), drop_rate=0., attn_drop_rate=0.,
        #                                drop_path_rate=0.1, num_frames=original_model.model.num_frames, attention_type=original_model.model.attention_type,
        #                                **kwargs)
    ...
    #shared_features = self.shared_blocks(x, B, T, W)
        ...
        player_scores = self.identity_head(x1[:, 0])
        #print("player_scores: ", player_scores.shape)
        action_scores = self.action_head(x2[:, 0])
        ...
        return player_scores, x1[:, 0], action_scores, x2[:, 0]