...
class Backbone(nn.Module):
    def __init__(self, args):
        ...
        self.corr_dim = args.corr_dim   # 64
...
class MultiCorrBackbone(nn.Module):
    ...
    def __init__(self, args):
        ...
        self.num_frames = args.num_frame    # 18
        self.num_channels = 512 if args.backbone in ('resnet18', 'resnet34') else 2048
        ...
        self.layer_channel = [64, 128, 256, 512]
        # self.motion_layer = 4
        self.channel_dim = self.layer_channel[self.motion_layer - 1]   # 512
        ...
        self.neighbor_size = args.neighbor_size  # 5
        self.ps = 2 * args.neighbor_size + 1   # 11 neighborhood p=11
    ...
    def forward(self, x):
        ...
        #print(x.size())   # torch.Size([36, 256, 45, 80])

        corr = self.get_local_corr(x, 0)
        #print(corr.size())    # torch.Size([36, 256, 45, 80])
        x = x + corr

        x = self.layer4(x)
        #print(x.size())  # torch.Size([36, 512, 23, 40])
        corr = self.get_local_corr(x, 1)
        #print(corr.size())  # torch.Size([36, 512, 23, 40])
        x = x + corr

        return x   # torch.Size([36, 512, 23, 40])
...
class Joiner(nn.Sequential):
    ...
    def forward(self, x):
        features = self[0](x)
        #features.size()  # torch.Size([36, 512, 23, 40])
        pos = self[1](features).to(x.dtype)
        #pos.size()   # torch.Size([36, 512, 23, 40])
